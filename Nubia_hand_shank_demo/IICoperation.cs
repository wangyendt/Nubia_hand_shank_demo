...
namespace Nubia_hand_shank_demo
{
    public class IICoperation
    {
        ...
        public static ushort[] ReadTemperature()
        {
            ...
            IIcWriteBuffer[1] = (byte)RegList.Reg.REG_TEMPERATURE;
            ...
            result[0] = (ushort)MyConvertor.byteArrToInt(IIcReadBuffer[1] << 8 | IIcReadBuffer[0]);
            result[1] = (ushort)MyConvertor.byteArrToInt(IIcReadBuffer[3] << 8 | IIcReadBuffer[2]);

            return result;
        }

        public static void DebugModeClean()
        {
            byte[] IIcWriteBuffer = new byte[256];
            byte[] IIcReadBuffer = new byte[256];
        ...
        public static object[] IICReadData(int channelNumber)
        {
            DebugModeClean();
            ...
            int[] rawdata = new int[channelNumber];
            int[] forceSig = new int[channelNumber];
            int[] touch_event = new int[1];

            if (IIcReadBuffer[0] == 4 * channelNumber + 12)
            {
                IIcWriteBuffer[1] = (byte)(RegList.Reg.REG_DEBUG_DATA1);
                ...
                int[] data = new int[(readLen - 2) / 2];

                //ushort crc_check = CRC_Check.calc_crc16(MyConvertor.byteArrToUint(IIcReadBuffer), readLen - 2);
                //ushort crc = (ushort)MyConvertor.byteArrToInt(IIcReadBuffer[readLen - 1] << 8 | IIcReadBuffer[readLen - 2]);

                //MessageBox.Show(crc + " " + crc_check);
                //if (crc == crc_check)
                {

                    for (int i = 0; i < (readLen - 2) / 2; i++)
                    {
                        data[i] = (short)MyConvertor.byteArrToInt(IIcReadBuffer[i * 2 + 1] << 8 |
                                                                   IIcReadBuffer[i * 2]);
                    }

                    Array.Copy(data, 0, threshold, 0, 1);
                    Array.Copy(data, 1, rawdata, 0, channelNumber);
                    Array.Copy(data, 5, forceSig, 0, channelNumber);
                    Array.Copy(data, 9, touch_event, 0, 1);
                }
            }
        ...
        public static object[] IICReadSingleResistanceList(int channelNumber)
        {
            bool bGetRes = false;
            ...
            IIcWriteBuffer[1] = (byte)RegList.Reg.REG_DEBUG_MODEB;
            IIcWriteBuffer[2] = (byte)RegList.DebugMode.DEBUG_MODE_READ_VBIAS_TWOSTATUS;
            ...
            int[,] iVBias = new int[channelNumber * 2, 3];
            ...
            do
            {
                ...
                count++;
            } while (IIcReadBuffer[0] == 0 && count < 100);

            if (IIcReadBuffer[0] == channelNumber * 8)
            {
                IIcWriteBuffer[1] = (byte)RegList.Reg.REG_DEBUG_DATAB;
                ...
                for (int iChn = 0; iChn < channelNumber; iChn++)
                {
                    iVBias[iChn * 2, 1] = Math.Max((short)1, (short)(IIcReadBuffer[4 * iChn] | (IIcReadBuffer[4 * iChn + 1] << 8)));
                    iVBias[iChn * 2 + 1, 1] = Math.Max((short)1, (short)(IIcReadBuffer[4 * iChn + 2] | (IIcReadBuffer[4 * iChn + 3] << 8)));
                    iVBias[iChn * 2, 0] = Math.Max((short)1, (short)(IIcReadBuffer[channelNumber * 4 + 4 * iChn] | (IIcReadBuffer[channelNumber * 4 + 4 * iChn + 1] << 8)));
                    iVBias[iChn * 2 + 1, 0] = Math.Max((short)1, (short)(IIcReadBuffer[channelNumber * 4 + 4 * iChn + 2] | (IIcReadBuffer[channelNumber * 4 + 4 * iChn + 3] << 8)));
                }
                bGetRes = true;
            }
            ...
            double[] dbR = new double[channelNumber * 4];
            int[] iR = new int[channelNumber * 4];
            double[] dbTotalR = new double[channelNumber];
            int[] iTotalR = new int[channelNumber];

            double dbR3 = 4700, dbVcc = 3000;

            for (int i = 0; i < channelNumber; i++)
            {
                dbR[i * 4 + 0] = dbR3 * (dbVcc / iVBias[i * 2, 1] - dbVcc / iVBias[i * 2, 0]);
                dbR[i * 4 + 1] = dbR3 * (dbVcc * (iVBias[i * 2, 0] - iVBias[i * 2, 1])) / ((dbVcc - iVBias[i * 2, 0]) * iVBias[i * 2, 1]);
                dbR[i * 4 + 2] = dbR3 * (dbVcc / iVBias[i * 2 + 1, 1] - dbVcc / iVBias[i * 2 + 1, 0]);
                dbR[i * 4 + 3] = dbR3 * (dbVcc * (iVBias[i * 2 + 1, 0] - iVBias[i * 2 + 1, 1])) / ((dbVcc - iVBias[i * 2 + 1, 0]) * iVBias[i * 2 + 1, 1]);

                iR[i * 4 + 0] = (int)dbR[i * 4 + 0];
                iR[i * 4 + 1] = (int)dbR[i * 4 + 1];
                iR[i * 4 + 2] = (int)dbR[i * 4 + 2];
                iR[i * 4 + 3] = (int)dbR[i * 4 + 3];

                dbTotalR[i] = (dbR[i * 4 + 0] + dbR[i * 4 + 1]) * (dbR[i * 4 + 2] + dbR[i * 4 + 3]) / (dbR[i * 4 + 0] + dbR[i * 4 + 1] + dbR[i * 4 + 2] + dbR[i * 4 + 3]);
                iTotalR[i] = (int)dbTotalR[i];
            }
